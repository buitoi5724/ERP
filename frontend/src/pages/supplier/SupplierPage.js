...
export default function SupplierPage() {
  ...
  const loadSuppliers = async () => {
    try {
      ...
      toast.current.show({
        severity: "error",
        summary: "Lỗi",
        detail: "Không thể load suppliers",
      });
    }
  };
  ...
  useEffect(() => {
    if (accounts.length > 0) loadSuppliers();
  }, [accounts]);
  ...
  const removeSelectedSuppliers = () => {
    confirmDialog({
      ...
      accept: async () => {
        await Promise.all(
          selectedSuppliers.map((s) => deleteSupplier(s.id))
        );
  ...
  const rightToolbarTemplate = () => (
    <div className="flex align-items-center gap-2">
      <span className="p-input-icon-left">
        ...
        <InputText
          placeholder="Tìm theo tên, SĐT, địa chỉ, MST, tài khoản..."
  ...
  return (
    <div className="supplier-container">
      ...
      >
        ...
        <Column field="taxCode" header="Mã số thuế" sortable />
        <Column header="Tài khoản" body={(row) => row.accountDisplay} sortable />
